...
// v1.1.6: force-apply shop defaults once (dia artifact & invite = unchecked; others checked) and set artifact m=5
(function(){
  function applyShopDefaultOnce(){
    const FLAG='ld_shop_defaults_v116';
    if (localStorage.getItem(FLAG)==='1') return;
    try{
      // set artifact numerator default to 5
      try{ localStorage.setItem('ld_artifact_m','5'); }catch(_){}
      if (typeof updateArtifactUI==='function'){
        try{ artifactCount = 5; updateArtifactUI(); }catch(_){}
      }
      const tbody = document.querySelector('#shop_table tbody');
      if (tbody){
        const rows = Array.from(tbody.querySelectorAll('tr'));
        for(const tr of rows){
          const key = tr.getAttribute('data-rowkey');
          const cb  = tr.querySelector('input.shop-check');
          const isDia     = tr.classList.contains('cur-dia');
          const isInvite  = tr.classList.contains('parent-invite');
          const isArtifact= !!(tr.querySelector('.item .icon')?.src||'').toLowerCase().includes('artifact');
          const shouldOn  = !(isDia && (isInvite || isArtifact));
          if (cb) cb.checked = shouldOn;
          if (typeof shopSelected!=='undefined' && shopSelected && typeof shopSelected.add==='function'){
            if (shouldOn) shopSelected.add(key); else shopSelected.delete(key);
          }
        }
      }
    }catch(_){}
    try{ if (typeof paintShop==='function') paintShop(shopRows); }catch(_){}
    try{ localStorage.setItem(FLAG,'1'); }catch(_){}
  }
  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(applyShopDefaultOnce, 0);
  });
})();
