console.log("app_v1_0_29.js loaded");
// app.js — v1.0.25
(function(){
  ...
  function updateArtifactUI(){
    ...
    if(lab) lab.innerHTML = `<span class=\"num\">${artifactCount}</span>/${ART_MAX}`;
  }
// ---- Shop ----
  let artifactCount = 0;
  ...
  function computeTotals(){
    ...
    for (const r of shopRows){
      ...
      if (r.item === "artifact"){
        if (r.price_currency === "gold") t.cost_gold += Math.round((r.price||0)*factor);
        else if (r.price_currency === "dia") t.cost_dia += Math.round((r.price||0)*factor);
      } else {
      ...
      t.artifact += (r.yield_artifact || 0) * factor; /*__ART_YIELD__*/
    }
    ...
    if(artVal>0){ if(sum.key){ sum.key.innerHTML = `${keyStr} <span class=\"plus-art\">(+${fmt1(artVal)})</span>`; } } else { setText(sum.key, keyStr); }
  ...
  function paintShop(rows){
    ...
    for (const r of rows){
      ...
      html += `<tr data-rowkey="${key}" class="${curClass} ${isInvite?'parent-invite':''}">`+
        ...
        `<td>${(r.item==='key'||r.item==='churu'||r.item==='battery'||r.item==='petfood'||r.item==='mythstone')?fmt1(qty):(r.item==='artifact'?fmt1(qty*((artifactCount||0)/31)):fmtInt(qty))}</td>`+
        `<td>${r.item==='artifact'?fmtInt(Math.round((r.price||0)*((artifactCount||0)/31))):fmtInt(r.price)}</td>`+
  ...
  async function fetchShop(){
    try{
      ...
      shopSelected.clear();
      for (const r of shopRows){
        const except = (r.price_currency === 'dia') && (r.item === 'invite' || r.item === 'artifact');
        if (!except) shopSelected.add(keyOf(r));
      }
      paintShop(shopRows);
